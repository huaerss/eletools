...
<template>
  <div>
    ...
    <div class="draggable" :style="{ '-webkit-app-region': isDraggable ? 'drag' : 'no-drag' }">
      <div>
        <p>{{ clipboardData }}</p>
        <div v-if="gptcontentvalue" style="border-top:1px  solid  brown;margin: 10px auto;width: 90%; ">
        </div>
        <p>{{ gptcontentvalue }}</p>
    ...
    <div style="-webkit-app-region:no-drag;position: absolute;top: 0;right: 0;" @click="changestatus">
      <svg v-if="isDraggable" xmlns="http://www.w3.org/2000/svg" style="margin: 4px 10px 0 0;color: brown;" width="20"
      ...
      <svg v-else style="margin: 4px 10px 0 0; color: crimson;" xmlns="http://www.w3.org/2000/svg" width="20"
        height="20" viewBox="0 0 24 24">
